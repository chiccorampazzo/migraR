...
rc_expression <-   function(profile = "eleven"){
      switch(profile,
        "rampazzo" = return(expression(log((a1 * exp(-alpha1 * x)) + (a2 * exp(-alpha2 * (x-mu2) - exp(-lambda2 * (x - mu2))))  + c1)))
       ...
       ,"seven" =  return(expression(log((a2 * exp(-alpha2 * (x-mu2) - exp(-lambda2 * (x - mu2)))) +
                               (a4 * exp(lambda4 * x)) + c1)))
      )
 }
